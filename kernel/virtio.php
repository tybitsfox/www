...
已用描述符表用于保存后端已经处理并且尚未反馈给驱动的描述。与可用描述符表不同的是，已用描述符表中数组ring的每个元素不仅包含后端设备已经处理的描述符链表的头部描述符在描述符表中的索引，而且由于后端设备可能会向前端驱动写回数据或需要告知
驱动写操作的状态，还需要包括一个len字段来记录设备写回数据的长度。已用描述符区域数据结构定义如下：
...
一个IO请求通常需要使用多个Descriptor描述，描述的信息一般包括一个请求、一个或多个数据页以及提供给后端设备填写处理结果的响应Buffer。通过flags中的VRING_DESC_F_INDIRECT字段是否设置，一个IO请求可以选择Chained Descriptor或者Indirector Descriptor
方式进行组织。
Chained Descriptor
...
Indirect Descriptor方式下，IO请求使用的描述符指向的是一个包含多个描述符的间接描述符表，并由间接描述表中的描述符描述所有的内存区域。对于一个IO请求，在包含的描述符数量较多的情况下，可以优化后端设备对描述信息的访问：设备可以一次性读取整个
间接描述表中所有描述符，避免沿着next字段逐个读取。
...
每次后端设备取用可用描述符时，需要知道剩余可用描述符在数组ring中的起始位置。后端设备会维护一个变量last_avail_idx，用来标记这个位置。当 切换 到主机中时，后端设备将检查last_avail_idx和idx的值，数组ring中位于last_avail_idx和idx-1之间的
部分就是可供后端设备使用的区域。
...
当设备驱动回收已用的设备描述符时，需要知道剩余已用标识符在数组ring中的起始位置，前端驱动会维护一个变量last_used_idx，用来标记这个位置。当切换到 虚拟机 中时，前端驱动将检查last_used_idx和idx的值，数组ring中位于last_used_idx和idx-1之间的
部分便是可供前端驱动回收的区域。
<center><img src=./virtio/34e973fb917c280aeffe616b29924053.png /></center>
Packed Virtqueue

Packed Virtqueue本质上是对Split Virtqueue的一种改进方案，在virtio协议 1.1版本中提出。在Split Virtqueue方案中，无论是前端驱动还是后端设备，都需要通过访问Driver Area或Device Area来确定需要访问的描述符，这不仅在数据结构管理上带来了复杂度，
同时也增加了设备DMA的开销。Packed Virtqueue在管理结构上进行了简化，驱动和设备所需要的信息都存放在了Descriptor Ring中。
