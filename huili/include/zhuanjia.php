...
<?php
//{{{右边显示栏抬头		div +6
		$st2="<a href='".$SIGNED_DEF['LINK']."' >主页</a></li><li>专家团队";
...
if(count($exp) > 0)
{
	...
	for($i=0;$i<5;$i++)
	{
		...
		$cy=array($ay[7],0);
		$s1=get_major($cy);
		$s2=$SIGNED_DEF['LINK']."?select=".$SIGNED_PAGE['GJ11']."&action=".$ay[0]."&vendor=0";
	...
	else
	{
		...
		$sta4=$SIGNED_DEF['LINK']."?select=".$SIGNED_PAGE['GJ11']."&prevb=".$bb."&vendor=0";
	}
	if($curr_pg[0] >= ($cnt[0]-1))
	{
	...
	else
	{
		...
		$sta3=$SIGNED_DEF['LINK']."?select=".$SIGNED_PAGE['GJ11']."&nextb=".$bb."&vendor=0";
	}
...
if(count($term) > 0)
{
	...
	for($i=0;$i<5;$i++)
	{
		...
		$cy=array($ay[7],1);
		$s1=get_major($cy);
		$s2=$SIGNED_DEF['LINK']."?select=".$SIGNED_PAGE['GJ11']."&action=".$ay[0]."&vendor=1";
		$st2=sprintf($st1,$ay[8],$ay[2],$i,$i,$ay[3],$s1,$ay[6],$s2);
		echo $st2;
	}
	echo "</ul>";
	...
	else
	{
		...
		$sta1="color: #3EAE48; text-decoration: none; border-bottom: 1px solid #3EAE48;";
		$sta4=$SIGNED_DEF['LINK']."?select=".$SIGNED_PAGE['GJ11']."&preva=".$bb."&vendor=1";
	}
	if($curr_pg[1] >= ($cnt[1]-1))
	{
		$sta2="color: gray; cursor: default; disabled: true;";
	...
	else
	{
		$bb=intval($curr_pg[1])+1;
		$sta2="color: #3EAE48; text-decoration: none; border-bottom: 1px solid #3EAE48;";
		$sta3=$SIGNED_DEF['LINK']."?select=".$SIGNED_PAGE['GJ11']."&nexta=".$bb."&vendor=1";
	}
...
if(count($sel_exp) > 0)
{
	...
	for($i=0;$i<5;$i++)
	{
		...
		$cy=array($ay[7],0);
		$s1=get_major($cy);
		$s2=$SIGNED_DEF['LINK']."?select=".$SIGNED_PAGE['GJ11']."&action=".$ay[0]."&vendor=2";
		$st2=sprintf($st1,$ay[8],$ay[2],$i,$i,$ay[3],$s1,$ay[6],$s2);
		echo $st2;
	}
	echo "</ul>";
	...
	else
	{
		...
		$sta1="color: #3EAE48; text-decoration: none; border-bottom: 1px solid #3EAE48;";
		$sta4=$SIGNED_DEF['LINK']."?select=".$SIGNED_PAGE['GJ11']."&preva=".$bb."&vendor=3";
	}
	if($curr_pg[2] >= ($cnt[2]-1))
	{
		$sta2="color: gray; cursor: default; disabled: true;";
	...
	else
	{
		$bb=intval($curr_pg[2])+1;
		$sta2="color: #3EAE48; text-decoration: none; border-bottom: 1px solid #3EAE48;";
		$sta3=$SIGNED_DEF['LINK']."?select=".$SIGNED_PAGE['GJ11']."&nexta=".$bb."&vendor=3";
	}
...
//开始专家的按条件选择界面
$sta3=$SIGNED_DEF['LINK']."?select=".$SIGNED_PAGE['GJ11']."&action=false&vendor=2";
...
if(count($sel_term) > 0)
{
	...
	for($i=0;$i<5;$i++)
	{
		...
		$cy=array($ay[7],1);
		$s1=get_major($cy);
		$s2=$SIGNED_DEF['LINK']."?select=".$SIGNED_PAGE['GJ11']."&action=".$ay[0]."&vendor=3";
		$st2=sprintf($st1,$ay[8],$ay[2],$i,$i,$ay[3],$s1,$ay[6],$s2);
		echo $st2;
	}
	echo "</ul>";
...
//echo "</div>";
//开始专家的按条件选择界面
$sta3=$SIGNED_DEF['LINK']."?select=".$SIGNED_PAGE['GJ11']."&action=false&vendor=3";
$st1="<form name='form2' method='post' action='".$sta3."'>
...
if($idt == 0)
{
	...
	for($i=0;$i<5;$i++)
	{
		...
		if(count($ay) == 3) //普通账户
		{
			...
			$cy[6]="单位";$cy[7]="";
			$cy[8]="";$cy[9]="";
			$cy[10]=$SIGNED_DEF['LINK']."?select=".$SIGNED_PAGE['GJ11']."&action=true&iid=".$ay[0];
		}
		else
		{
			$cy[0]=$ay[8];//头像
	...
	else
	{
		...
		$sta1="color: #3EAE48; text-decoration: none; border-bottom: 1px solid #3EAE48;";
		$sta4=$SIGNED_DEF['LINK']."?select=".$SIGNED_PAGE['GJ11']."&prevc=".$bb."&action=true";
	}
	if($curr_pg[4] >= ($cnt[4]-1))
	{
		$sta2="color: gray; cursor: default; disabled: true;";
	...
	else
	{
		$bb=intval($curr_pg[4])+1;
		$sta2="color: #3EAE48; text-decoration: none; border-bottom: 1px solid #3EAE48;";
		$sta3=$SIGNED_DEF['LINK']."?select=".$SIGNED_PAGE['GJ11']."&nextc=".$bb."&action=true";
	}
