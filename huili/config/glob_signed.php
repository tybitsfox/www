...
$SIGNED_PAGE['GJ10']		=	md5('fagui');
//$SIGNED_PAGE['GJ13']		=	md5('xiazai');
$SIGNED_PAGE['GJ11']		=	md5('zhuanjia');
$SIGNED_PAGE['GJ12']		=	md5('pingtai');
...
$SIGNED_DEF['MODULE']	= array(
	...
	//	array($SIGNED_DEF['LINK']."?select=".$SIGNED_PAGE['GJ11'],"技术动态","with-name","chlink12","icon-joomla",$SIGNED_PAGE['GJ11']),
		array($SIGNED_DEF['LINK']."?select=".$SIGNED_PAGE['GJ10'],"环境法规","with-name","chlink13","icon-wpforms",$SIGNED_PAGE['GJ10']),
	//	array($SIGNED_DEF['LINK']."?select=".$SIGNED_PAGE['GJ13'],"资料下载","with-name","chlink14","icon-download",$SIGNED_PAGE['GJ13']),
		array($SIGNED_DEF['LINK']."?select=".$SIGNED_PAGE['GJ11'],"专家团队","with-name","chlink15","icon-group",$SIGNED_PAGE['GJ11']),
		array($SIGNED_DEF['LINK']."?select=".$SIGNED_PAGE['GJ12'],"监控平台","with-name","chlink16","icon-desktop",$SIGNED_PAGE['GJ12']));
...
//右边默认界面及默认界面的展开代码
$SIG_HTML['RIGHT_TOP2']	=	"<div class='tab-content'><div role='tabpanel' class='tab-pane tab-pane-naked active' id='latestorders'><section><div class='orders-empty panel'><img class='picto' src='".$SIGNED_DEF['PICTO_PNG']."' alt='空'><p>%s</p><a id='btslide1' href='#' class='btn btn-primary withlasticon'>选择功能模块<i class='icon-plus'></i></a></div></section></div></div>";
