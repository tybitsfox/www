<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
<?php
echo "<center><font size=5 color=red>Ubuntu 20.04安装笔记</font></center>";
echo "2022-8-29 setup ubuntu22.04升级或下载总是提示：连接失败,IP:91.189.91.39 80,主镜像或cn.archive.ubuntu.com都是国外服务器，被狗日的墙了<br>换个国内的源服务器吧，<font size=6 color=red>FUCK YOU CCP!!!</font><br>";
echo "<center><font size=5 color=red>关于nvidia GF730显卡和ubuntu各版本兼容性的问题</font></center>";
echo "2025-12月1日，前几天因为安装某个python3脚本库，更新了下我的ubuntu系统，因为在这之前我已知道新版ubuntu对老旧的nvidia显卡驱动支持有限，实际上是nvidia这个煞笔公司提供的闭源驱动与linux6.5以后的新内核存在兼容问题，而nvidia也懒得再提供老旧显卡的继续支持。所以ubuntu24.04的版本虽然带有对GT730的闭源驱动，但无论那种方式都不能安装成功。对此我已做了多次测试。所以之前升级到24.04版本的我只能回到ubuntu22.04版了。但是目前因为ubuntu22.04也一直不断的更新，现在22.04的最新版（ubuntu22.04.4）也无法使用GT730的闭源驱动了，初步测试ubuntu22.04.2版本还能使用nvidia的闭源驱动。之所以纠结这个问题，是因为我在ubuntu上使用的yuzu模拟器还有retroarch游戏模拟器都需要nvidia的闭源驱动才行！这次重装系统整整折腾了两天，一个周末搭进去了，重装了得5,6遍，测试了各种版本：22.04.4，20.04.6，22.04，24.04.3，22.04.2。因此也把ubuntu的更新和版本差异弄明白了。就目前测试情况看，ubuntu22.04.2之前的版本，nvidia提供的闭源驱动都能完美运行。在ubuntu论坛上也看了各种同样问题的帖子，也有各种安装方法，其中比较权威的说法就是nvidia提供的390.xxx和470.xxx闭源驱动在linux6.5内核以后将不再使用。在ubuntu23.04版本上直接去掉了390版本的驱动，虽然还带有470版本的驱动但也无法安装成功。看来要使用nvidia的闭源驱动只能选择老版本的ubuntu了，虽然老版本驱动支持很好，但因其基础的c运行库版本老旧，对于新的模拟器也无法满足其运行，我在ubuntu20.04.6版本上就碰到了这个问题，虽然显卡没有问题，但模拟器因为stdlibc++和libc这个c运行库版本太低也不能运行。so，现在能满足我要求的就只能是ubuntu22.04~22.04.2这个范围的版本了，可问题还没完全解决，因为这个范围的版本已不是稳定版了，官方下载也只能在old-release页面才能找到，并且这个版本在使用中绝对不能进行更新，只要一更新立马更新到稳定版（目前是22.04.4）显卡也立马就挂。不更新看似简单，可一旦我需要安装某个新应用，如果不更新到稳定版，要么因为各种库依赖无法安装，要么因当前版本是unstable而不提供安装。真的是左右为难了。";


echo "<br><font color=blue size=5>64位ubuntu22.04下编译执行32位程序</font><br>
<pre>首先确认你有一个64位架构的内核： dpkg --print-architecture
64位系统会显示：amd64
然后确认你打开了多架构支持功能：dpkg --print-foreign-architectures
若输出：i386，则系统已经打开了多架构支持，否则：sudo dpkg --add-architecture i386;	sudo apt -get update
然后安装支持的32位运行库：需要安装的库：lib32z1,lib32z1-dev
这里说明一下：原来的32位运行库为ia32-libs，自ubuntu13.1之后该库被剔除，改用:lib32z1 lib32ncurses5 lib32bz2-1.0之一替代即可。
编译32位c代码时还需要以下gcc库支持：sudo apt-get install gcc-multilib g++-multilib</pre><br><br>";
echo "<font size=5 color=red>Ubuntu20.04系统重启时黑屏的解决办法</font><br>";
echo "<pre>
首先在黑屏界面使用Ctrl+Alt+F2进入命令行模式，先在login后面输入刚进去时设置的用户名按enter，再输入密码enter，发现成功进入系统！这说明系统运行
完全正常！那么系统黑屏的原因很大可能就是Ubuntu系统自带的Nouveau显卡驱动和Nvidia显卡不兼容了。这时候可以在命令行后面输入：
sudo vim /etc/default/grub
找到quiet splash并在后面添加nomodeset
然后输入下面指令，进行grub配置文件更新
sudo update-grub
重启系统后应该就可解决问题了。
</pre>";
echo "<font size=4 color=blue><pre>
install:
aptitude
vim-gtk3
ctags

------------------------------
edit .bashrc
cp .vim/ .vimrc
cp -r winfonts/ to /usr/share/fonts/truetype/
change video card driver to nvidia 460
reboot
edit /boot/grub/grub.cfg
----------------------------------------------
install:
apache2
mariadb10.3-server(client)
php7.4
php-mysql
libapache2-mod-php7.4 (可能已经安装)
php7.4-gd
导入数据库:
web_data:	taenv	taenv2014;
gis_hb2018: sdhl	sdhl2016;
create database web_data;
use web_data;
source /home/tian/web_data.sql;
create user 'taenv'@'localhost' identified by 'taenv2014';
grant all on web_data.* to 'taenv'@'localhost';
----------------------------------------------------
edit /etc/apache2/sites-available/default-ssl.conf
edit /etc/apache2/sites-available/000-default.conf
修改默认的网站目录为：/var/www
进入/etc/init.d/重启服务：./apache2 restart
拷贝www目录至/var/ 网站设置完成!
----------------------------------------------------
界面美化：
sudo apt install gnome-tweaks
打开网站：https://extensions.gnome.org/
安装：'User Themes'和'Dash to Dock'
然后打开'扩展'，禁用ubuntu dock,启用并配置dash to dock,重启完成！
----------------------------------------------------------------
继续安装我常用的软件：
git
vlc
openssh-server
kernel-wedge
libgtk2.0-dev
libgtk-3-dev
zathura
mednafen
scrot
feh
xterm
fluxbox
pcsx2:i386
desmume

---------------------------------------------------------------
xmame所需运行库:
libXv.so.1 => sudo apt install ibxv1:i386
缺少声音模块：snd-pcm-oss.ko 没有声音
牛逼的不要不要！哈哈，成功将xmame完美运行在ubuntu20.04上了！！；）
由于自ubuntu10以后snd-pcm-oss.ko就已经被系统舍弃，因此对于使用老旧的oss声音
的软件就无法正常的通过加载上述模块使用声音了。通过网上查询，终于找到了一个可
行的方法：安装libpulsedsp,这个库可以将使用oss输出的软件，通过pulseaudio模拟
输出！因为我使用的还是32位的xmame，因此应安装32位版本的库文件：libpulsedsp:i386 
(关于osspd.conf，这个文件在/etc/modprobe.d中，应该是必须的配置文件，之前我测试的方法很多，以至于在这个目录下产生了多个配置文件，还导致了声音无法打开的情况：声音设置中设备显示的是伪输出，配置多是oss4的：
 ss4-base.conf,oss4-base_noOSS3.conf,oss4-base_noALSA.conf,可以确定伪输出的产生就是这些配置造成的。
 osspd.conf:
blacklist snd-pcm-oss
blacklist snd-mixer-oss
blacklist snd-seq-oss)

安装完成后需使用脚本文件实现加载：
xmame.sh
#!/bin/bash
export LD_PRELOAD='/usr/lib/i386-linux-gnu/pulseaudio/libpulsedsp.so'
/workarea/home/tian/games/xmame/xmame $1

通过这个脚本，即可完美实现xmame在系统上的重生！；）
---------------------------------------------------------------
自此，我所需要的东西基本都安装完成了，还剩一个最喜爱的ps1模拟器：ePSXe
这个软件也是一款老旧软件了，但是他无可比拟的性能让pcsxr等新生代ps1模拟器
无法望其项背！虽然有64位版本，但是其所需的64位运行库我无法找到了。而32位
的运行库因为之前在debian中一直使用，所以都得以保存。拷贝过来，还需安装一些
i386下的库文件，为了将来着想，将其一并记录保存下来：

libncurses.so.5
libtinfo.so.5
libSDL-1.2.so.0
libSDL_ttf-2.0.so.0
libnghttp2.so.14
librtmp.so.1
libssh2.so.1
libcaca.so.0
libsasl2.so.2
libslang.so.2
libncursesw.so.6
完美的库文件都以放置在/usr/local/lib/old_lib目录下</pre></font>
";
echo "<br><br><center><font size=5>AMD显卡的驱动安装</font></center>";
echo "<font size=4><pre>
一、相关信息查询命令：
sudo lshw -c video
lsmod | grep amd
dmesg | grep amd
二、安装驱动方法
1、使用开源驱动，无需多说，系统安装时会自动安装
2、使用第三方仓库，安装amd官方驱动
添加私有源：sudo add-apt-repository ppa:oibaf/graphics-drivers
然后执行如下命令：sudo apt update && sudo apt -y upgrade 即可
保险其间，如果想卸载私有源驱动，可使用：
sudo apt install ppa-purge
sudo ppa-purge ppa:oibaf/graphics-drivers
3、使用源代码直接编译
进入amd主页，下载源代码，执行如下命令解压安装：
tar -xf amdgpu-pro_*.tar.xz
cd amdgpu-pro-XX.XX-XXXXXX
./amdgpu-pro-install -y
</pre></font><br><br>";
echo "ubuntu使用fluxbox时挂载u盘出现乱码的解决：<br>
mount -t vfat -o iocharset=utf8 /dev/sdc1 /mnt<br><br><br><br><br>";
?>
