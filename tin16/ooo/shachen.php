<?php
echo "<html><head>
<meta http-equiv='Content-Type' content='text/html; charset=utf-8' />
</head><body>";
echo "<br><center><font size=6 color=red>沙尘之锁攻略</font></center><br>";
echo "<center><img src='./60748e13632762d01408e245aaec08fa503dc6ab.jpg ' border=1 style='width:50%;cursor:pointer' onclick='javascript:window.open(this.src)' /></center><br>";
echo "<table align=center width=70% border=1 cellspacing=0 cellpadding=1><tr>";
echo "<td align=center colspan=2><a href='./walk.php' target=_blank><font size=4>流程篇</font></a></td>";
echo "<td align=center colspan=2><a href='./walk.php#vvv' target=_blank><font size=4>大地图</font></a></td></tr><tr>";
echo "<td align=center><a href='./hecheng1.php#pic01' target=_blank><font size=4>人类武器合成</font></a></td>";
echo "<td align=center><a href='./hecheng1.php#pic02' target=_blank><font size=4>主炮合成</font></a></td>";
echo "<td align=center><a href='./hecheng1.php#pic03' target=_blank><font size=4>副炮合成</font></a></td>";
echo "<td align=center><a href='./hecheng1.php#pic04' target=_blank><font size=4>SE合成</font></a></td></tr><tr>";
echo "<td align=center colspan=2><a href='./hecheng1.php#pic05' target=_blank><font size=4>特殊材料合成</font></a></td>";
echo "<td align=center colspan=2><a href='./walk.php#hecheng' target=_blank><font size=4>极品垃圾交换物</font></a></td></tr>";
echo "</table>";
?>
